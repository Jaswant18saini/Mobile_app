...
const Sync = () => {
  ...
  const handleDataSyncStorage = async (docId, index) => {
    ...
    if(savedData) {
      ...
      try {
        await AsyncStorage.setItem('OfflineAnnotations', JSON.stringify(savedDataJson));
  ...
  return (
    <View>
      ...
      <Text>Annotations</Text>
        ...
        <View>
          <Button title='Sync Annotations' disabled={!netInfo.isInternetReachable} onPress={handleSyncAnnotations} />
        </View>
      </View>
    </View>
  );
